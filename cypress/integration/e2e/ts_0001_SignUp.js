...
describe("TC_SU_001 Validate Sign-Up an Account by providing only the Mandatory fields", () => {
  it("Should be able to submit a successfull submission via Sign Up form", () => {
    ...
    cy.get(
      ".MuiDialogActions-root > a > .MuiButtonBase-root > .MuiButton-label"
    ).click();
  });
}); //End of TC_SU_001
...
//TC_SU_006
describe("TC_SU_006 Validate Sign Up error message when providing Invalid Email address", () => {
  it("Should not be able to submit a successfull submission via Sign Up form", () => {
    cy.visit("http://localhost:3000/signup");
    cy.get('[id="name"]').type("User Alpha");
  ...
  it("User should click on the SUBMIT button and error message need to be presented", () => {
    cy.get(
      ".MuiCardActions-root > .MuiButtonBase-root > .MuiButton-label"
    ).click();
    //validate error message content
    cy.get("p").should(($p) => {
      //make sure the element contains text content
      expect($p.last()).to.contain("Please fill a valid email address");
    });
  });
}); //End of TC_SU_006
